...
function renderMessages(data) {
  ...
  data.forEach(msg => {
    ...
    `;
    div.addEventListener('click', () => showToast(`Opening message from ${msg.initials}…`));
...
function setupNav() {
  ...
  items.forEach(item => {
    item.addEventListener('click', e => {
      ...
      if (page !== 'dashboard') showToast(`Navigating to ${item.textContent.trim()}…`);
    });
  });
}
...
function setupSearch() {
  ...
  input.addEventListener('input', () => {
    ...
    debounce = setTimeout(() => {
      ...
      if (!q) { renderQueue(QUEUE_DATA); return; }
      const filtered = QUEUE_DATA.filter(
        r => r.name.toLowerCase().includes(q) || r.reason.toLowerCase().includes(q)
      );
...
function setupButtons() {
  document.getElementById('nextConsultBtn')?.addEventListener('click', () => {
    ...
    showToast(next ? `Starting consultation for ${next.name}…` : 'No scheduled patients remaining.');
  });

  document.getElementById('emergencyLeaveBtn')?.addEventListener('click', () => {
  ...
  document.getElementById('startConsultationBtn')?.addEventListener('click', () => {
    const next = QUEUE_DATA.find(r => r.status === 'scheduled');
    showToast(next ? `Starting consultation for ${next.name}…` : 'No scheduled patients.');
  });
}
